...
export async function startExpress(opts: Options) {
  ...
  const slowdown = slowDown({ windowMs: 60_000, delayAfter: 250, delayMs: 250 });
  ...
  promClient.collectDefaultMetrics({ register: registry, prefix: `${opts.serviceName}_` });
  ...
  app.get('/health', (_req, res) =>
    res.json({ status: 'ok', service: opts.serviceName, time: new Date().toISOString() }),
  );
  ...
  const server = httpServer.listen(port, () => rootLogger.info(`🚀  ${opts.serviceName} on :${port}`));
